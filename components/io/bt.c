...
#define EXAMPLE_DEVICE_NAME "ESP-lisp"
    ...
    switch (event) {
        ...
        case ESP_SPP_DATA_IND_EVT:
            ...
            for (size_t i=0; i<param->data_ind.len; i++) {
                if (xQueueSend(*bt_io_data.in_buff, &param->data_ind.data[i], 0) != pdPASS) {
                    ESP_LOGE(SPP_TAG, "Bluetooth buffer overflow");
                    abort();
                }
            }
            break;
...
void init_bt(void)
{
    ...
    xQueue = xQueueCreate(1024, sizeof(char));
    bt_io_data.in_buff = &xQueue;
...
static int bt_read(struct io_primitive *buff) {
...
static struct io_primitive bt_hndlr = {
    .read = bt_read,
    .write = bt_write,
    .private = (void*)&bt_io_data,
};

PORTS_SECTION struct io_typ bt_port = {
    .private = &bt_hndlr,
    .read = text_read,
    .write = text_write,
    .proto = "bt",
...
MODULE_SECTION struct module bt_module = {
    .setup = init,
};
